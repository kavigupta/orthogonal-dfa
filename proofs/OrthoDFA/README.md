...
| `Oracle` (structure) | `Liveness.lean` | The persistent oracle as random classification noise, indexed by the **query string** `w : W` (not a (prefix,suffix) pair): `noise w` a {0,1} bit, iid `Bernoulli(η)` over distinct strings (bit-valuedness asserted, `[0,1]` derived); `q i v` the concatenation `x_i·v` with `q_inj` (distinct prefixes → distinct strings); `flip` the language. The read `= flip ⊕ noise(q i v)` and its mean/independence (via `iIndepFun.precomp`)/range are **derived**, not fields. |
| `greedy_picks_good` | `Liveness.lean` | **Clean-premise liveness.** Takes one `Oracle` + language/findability — no `trigger`/`himplies`, no asserted read law. Loss means derived from the oracle; the greedy's least-loss `k`-subset avoids the bad set (flip ≥ ε_cov) except w.p. `#cands·exp(−2m·((½−η)ε_cov)²)`. |

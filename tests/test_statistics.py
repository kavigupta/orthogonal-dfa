...
class TestPopulationSizing(unittest.TestCase):
    def test_low_signal_is_sized_rather_than_diverging(self):
        ...
        for signal in SIGNALS:
            N, eps = population_size_and_evidence_margin(signal, 0.01, 0.01)
    ...
    def test_size_scales_as_the_inverse_square_of_the_signal(self):
        ...
        for signal in [s for s in SIGNALS if s <= 0.2]:
            N, _ = population_size_and_evidence_margin(signal, 0.01, 0.01)
            N_half, _ = population_size_and_evidence_margin(signal / 2, 0.01, 0.01)
    ...
    def test_size_is_minimal(self):
        for signal in [0.3, 0.05, 0.01]:
            N, _ = population_size_and_evidence_margin(signal, 0.01, 0.01)
            self.assertIsNone(
                evidence_margin_for_population_size(signal, 0.01, 0.01, N - 1), signal
            )

    def test_unseparable_signal_reports_rather_than_overflows(self):
        with self.assertRaisesRegex(NoUsableEvidenceMargin, str(MAX_POPULATION_SIZE)):
            population_size_and_evidence_margin(1e-6, 0.01, 0.01)

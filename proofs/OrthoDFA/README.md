...
| `greedy_picks_good` | `Liveness.lean` | **Clean-premise liveness.** All premises are oracle / language / findability — no `trigger`/`himplies`. The loss means are derived from the oracle (`η + (1−2η)·flip`); the greedy's least-loss `k`-subset avoids the bad set (flip ≥ ε_cov) except w.p. `#cands·exp(−2m·((½−η)ε_cov)²)`. |

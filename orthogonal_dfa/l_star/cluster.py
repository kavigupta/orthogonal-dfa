...
def sample_suffix_family(pst, v: int, state) -> Tuple[List[int], float]:
    ...
    while True:
        ...
        #
        # The suffixes were screened at whatever boundary stood when each was
        # drawn, so once this one moves, the pool is screened again and clustered
        # afresh.  Retirement only shrinks the pool, so this settles.
        attempts = 0
        while True:

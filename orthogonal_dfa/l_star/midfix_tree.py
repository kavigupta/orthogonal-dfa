...
Internal nodes are *midfixes*.  A node's midfix ``p`` classifies a string ``s`` by
the family membership of ``s + p + v`` over the round's base suffixes ``v`` -- so
``p`` sits between the string and each suffix, hence the name.  Leaves are DFA
state ids.

The tree owns *structure* only: which midfix cuts where, and which leaf a branch
lands in.  Reading a node is delegated to a caller-supplied decision callback --
against the oracle for a fresh string, or against the cached mask matrix for the
whole prefix pool -- so the thresholds and the query mechanism stay with the
caller.

Splitting only ever refines a leaf.  The accept side of the root stays the accept
side forever, so ``accepting_leaves`` needs no per-node labels; and the True (accept)
branch reuses the split leaf's id, so every id in ``range(num_states)`` is always a
live leaf and the ids need no remapping on export.
"""
...
class MidfixTree:
    ...
    def __init__(self, base_family: List[List[int]]):
        # The base distinguisher family the midfixes are read against.
    ...
    def split(self, state: int, midfix) -> int:
        ...
        ``midfix``, returning the new state id.  The True branch reuses the old id
        so existing references to ``state`` stay valid."""
    ...
    def classify(self, seq, decide: Decide) -> Optional[int]:
        """Route ``seq`` to a leaf, or ``None`` when a node's ``decide`` abstains."""
        node = self._root
    ...
    def classify_many(self, seqs, decide_level) -> List[Optional[int]]:
        """``[self.classify(s, ...) for s in seqs]`` with the reads batched one
        level at a time: ``decide_level(pairs)`` gets every ``(seq, midfix)`` at the
        current level and returns their decisions, so a batching oracle spends one
        call per level rather than one per node.  A ``None`` decision drops that
        string (its result stays ``None``)."""
    ...
    def classify_pool(self, num_prefixes: int, decide_columns) -> np.ndarray:
        """Classify a whole fixed population at once.  ``decide_columns(midfix)``
        returns the ``(accept, reject)`` boolean columns over all ``num_prefixes``
        for that node's family -- read straight off a cached mask matrix, no oracle.
        A prefix an ancestor abstained on stays ``-1``."""
...
def oracle_decider(oracle, base_family: List[List[int]], accept: float, reject: float):
    """A ``(decide, decide_level)`` pair that reads a midfix node against ``oracle``.

    Both query ``s + midfix + v`` over ``base_family`` and threshold the mean the
    same way (``> accept`` accepts, ``< reject`` rejects, the band between abstains);
    ``decide`` scores one string, ``decide_level`` scores a whole level in one
    batched call for :meth:`MidfixTree.classify_many`."""

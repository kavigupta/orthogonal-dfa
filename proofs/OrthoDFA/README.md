...
| `denoised_loss_eq_flip` | `Liveness.lean` | **Loss ↔ flip-mass.** Against the denoised oracle, mean loss over `m` prefixes `= m·c₀ + 2s·(flip count)` — linear in the flips. Discharges the mean-separability hypotheses (`ρlo = c₀`, `ρhi = c₀ + 2s·ε_cov`, `γ = s·ε_cov`) from the per-prefix read model, making the modeling identification explicit. |

...
one string at a time.  Concatenation and flipping are not fields: strings are Mathlib's
`[Mul S]`, and `flip` is derived below. -/
...
/-- The membership query, `MQ w = ℓ(w) ⊕ noise(w)`.
...
error budget.  Where a field's condition mentions the share it will be given — which depends
on that field — it mentions it only through *its* logarithm, and `log x ≤ 2√x` closes the
loop in one step. -/

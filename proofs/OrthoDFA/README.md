...
| `Oracle` (structure) | `Liveness.lean` | The persistent oracle bundled as random classification noise: `noise` bits (iid `Bernoulli(η)`, `[0,1]`, `mean = η`) + the language's `flip`. The disagreement `read = flip ⊕ noise` and its mean/independence/range are **derived** (`read_mean`, `read_indep`, `read_icc`), not fields. |

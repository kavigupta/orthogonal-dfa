...
theorem measureReal_noCleanRef_le (D : J → Measure S) (Dsf : Measure S)
  ...
  classical
  set A : Set (Run Ω S J) := {x | (((Finset.univ : Finset (Fin B.nsuff)).filter
      (fun i => ∀ p, O.label (p * suffixDraw i.val x) = O.label p)).card : ℝ)
      ≤ (B.nsuff : ℝ) * (pAP - t)} with hA
  set N : Set (Run Ω S J) :=
  ...
  have hsub : {x : Run Ω S J | ¬ ∃ w₀ ∈ (poolAt B.nsuff x).erase 1,
    ...
    intro x hx
    by_contra hnot
    simp only [Set.mem_union, not_or, hA, hN, Set.mem_setOf_eq, not_not] at hnot
          ...
          · -- the certification sample is good and the family is clean
            ...
            have hmass : (D j).real {p | ¬ (flipCount O
              ...
              · have hsetle : {p : S | ¬ (flipCount O F p ≤ ((F.card : ℝ)) * 0)}
                ...
                nlinarith
            by_cases h7 : (B.npref : ℝ) * (Δp * (kmax : ℝ) + th)
                ≤ (((certOf j B.npref x).filter (fun p =>
                  ¬ (flipCount O ((clusterAt O.mq populations x B).erase 1) p
...
lemma solved_roundFail (η₀ : ℝ) (populations : Finset J)
  ...
  refine le_trans (mul_le_mul_of_nonneg_left
    (c := 10 * ε₀ + (((populations.card : ℝ) + 3) * (m : ℝ) ^ 2 * ρ + 2 * (M : ℝ) ^ 2 * ρsf))
    ?_ hcard.le) ?_
...
theorem exists_passable (O : Oracle μ S) (populations : Finset J) (D : J → Measure S)
  ...
  refine ⟨B, Finset.mem_filter.2 ⟨?_, ⟨?_, ?_, ?_⟩⟩,
    ...
    sig η₀ / 2, screenMargin η₀ populations εcov δ / 2,
...
theorem loop_terminates {Pre : Set S} (hflat : Flat Pre) (O : Oracle μ S)
  ...
  calc ∑ j ∈ populations, (runMeasure μ D Dsf).real
        {x : Run Ω S J | x ∉ retAt O η₀ populations indecisionLimit εcov α B j}
      ≤ ∑ _j ∈ populations,
          roundFail populations l lcut τ th E γscr γdirty gdirty tap ρ (collisionMass Dsf) ⌊(1 - indecisionLimit) * (B.npref : ℝ)⌋₊ B :=

...
| `Oracle` (structure) | `Liveness.lean` | The persistent oracle as random classification noise, indexed by the **query string** `w : W` (not a (prefix,suffix) pair): `noise w` iid `Bernoulli(η)` over distinct strings; `q i v` the concatenation `x_i·v` with `q_inj` (distinct prefixes → distinct strings); `flip` the language. The read `= flip ⊕ noise(q i v)` and its mean/independence (via `iIndepFun.precomp`)/range are **derived**, not fields. |

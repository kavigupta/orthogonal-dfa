"""Tests for the template filler on its own: holes, forbidden patterns and the
uniformity of the draw, with no vocabulary or parser in sight.
"""
...
class TestContract(unittest.TestCase):
    ...
    def test_duplicate_and_prefix_related_patterns_are_fine(self):
        ...
        for seed in range(50):
            out = filler.fill([FREE, 1], np.random.default_rng(seed))
...
class TestFillability(unittest.TestCase):
    def test_every_context_is_fillable(self):
